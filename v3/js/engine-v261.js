(function initEngineV261(global) {
  ...
  const NREL_CACHE_STORAGE_KEY = "solarUpgradeV3.nrelCache.v1";
  ...
  function getHaMonthDispatchConfig(ha, monthIndex) {
    ...
    return {
      ...
      haMaxShiftHoursPerDay: Math.max(0, Math.floor(Number(ha.maxShiftHoursPerDay) || 0))
    };
  }
  ...
  function buildSimulationInputs(rawInputs, climateSnapshot) {
    const climateProfile = climateSnapshot && isValidClimateProfile(climateSnapshot.profile)
    ...
    return {
      ...
      production: {
        ...
        tempHourlyFByMonth: climateProfile.tempHourlyFByMonth,
      ...
      load: {
        ...
        summerSetpointF: BASE_SUMMER_SETPOINT_F,
        winterSetpointF: BASE_WINTER_SETPOINT_F
      },
      ev: {
        enabled: false,
        milesPerMonth: 0,
      ...
      ha: {
        enabled: false,
        mode: "auto",
        successRateByMonth: new Array(12).fill(0),
        autoPreWindowByMonth: new Array(12).fill(0).map(() => ({ startHour: 12, endHour: 16 })),
        tempUnits: "F",
        summerSetpointF: BASE_SUMMER_SETPOINT_F,
        winterSetpointF: BASE_WINTER_SETPOINT_F,
        maxPrecoolOffsetF: 0,
        maxPreheatOffsetF: 0,
        maxPeakRelaxOffsetF: 0,
        hvacSensitivityKwhPerDegHour: 0.60,
        successRate: 0,
        preCoolStartHour: 12,
        preCoolEndHour: 16,
        maxShiftHoursPerDay: 0,
        maxShiftKwhPerDay: 0,
      ...
      whf: {
        ...
        successRate: 0,
        successRateByMonth: new Array(12).fill(0),
        autoWindowByMonth: new Array(12).fill(0).map(() => ({ startMinuteOfDay: 0, endMinuteOfDay: 0 })),
        autoActiveMonths: []
      },
      rates: {
        tariffPreset: "custom",
      ...
      financing: {
        apr: Math.max(0, asFinite(rawInputs.financing.aprPct, 6)) / 100,
      ...
      climate: {
        status: climateSnapshot ? climateSnapshot.status : "fallback_synthetic",
        locationLabel: climateSnapshot ? climateSnapshot.locationLabel : "",
        lastVerifiedAt: climateSnapshot ? climateSnapshot.lastVerifiedAt : null,
        fallbackReason: climateSnapshot ? climateSnapshot.fallbackReason : "missing_data",
